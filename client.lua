--[[.----------------.  .----------------.  .-----------------. .-----------------. .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| | ____   ____  | || |     _____    | || | ____  _____  | || | ____  _____  | || |  ____  ____  | |
| ||_  _| |_  _| | || |    |_   _|   | || ||_   \|_   _| | || ||_   \|_   _| | || | |_  _||_  _| | |
| |  \ \   / /   | || |      | |     | || |  |   \ | |   | || |  |   \ | |   | || |   \ \  / /   | |
| |   \ \ / /    | || |      | |     | || |  | |\ \| |   | || |  | |\ \| |   | || |    \ \/ /    | |
| |    \ ' /     | || |     _| |_    | || | _| |_\   |_  | || | _| |_\   |_  | || |    _|  |_    | |
| |     \_/      | || |    |_____|   | || ||_____|\____| | || ||_____|\____| | || |   |______|   | |
| |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
'----------------'  '----------------'  '----------------'  '----------------'  '----------------' --]]
...
Citizen.CreateThread(function()
 	while true do
		...
		if IsControlPressed(0, Keys["SHIFT"]) then
			if IsControlJustReleased(0, Keys["N6"]) then -- numpad 6
			repeat
			fnTable[1]();
			Utils.Sleep(fnTable[7]);
			Utils.Notify('inform', 'Stunt Ready: Left Nac Nac!')
			until not IsControlJustReleased(0, Keys["N6"])
			elseif IsControlJustReleased(0, Keys["N4"]) then -- numpad 4
			repeat
			fnTable[2]();
			Utils.Sleep(fnTable[7]);
			Utils.Notify('inform','Stunt Ready: Right Nac Nac!')
			until not IsControlJustReleased(0, Keys["N4"])
			elseif IsControlJustReleased(0, Keys["N5"]) then -- numpad 5
			repeat
			fnTable[3]();
			Utils.Sleep(fnTable[7]);
			Utils.Notify('inform', 'Stunt Ready: No Hand Zone!')
			until not IsControlJustReleased(0, Keys["N5"])
			elseif IsControlJustReleased(0, Keys["N8"]) then -- numpad 8
			repeat
			fnTable[4]();
			Utils.Sleep(fnTable[7]);
			Utils.Notify('inform', 'Stunt Ready: Hop Scotch!')
			until not IsControlJustReleased(0, Keys["N8"])
			elseif IsControlJustReleased(0, Keys["N7"]) then -- numpad 7
			repeat
			fnTable[5]();
			Utils.Sleep(fnTable[7]);
			Utils.Notify('inform', 'Stunt Ready: Egyptian!')
